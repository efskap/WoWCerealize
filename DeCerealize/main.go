...
func (d *DeCerealizer) Start(out chan <- string) {
	// align loop with system clock on the second
	timeUntilSecond := time.Until(time.Now().Add(time.Second).Truncate(time.Second))
	time.Sleep(timeUntilSecond)
	for {
...
func (e *ChecksumError) Error() string { return fmt.Sprintf("checksum error (%d != expected %d)", e.got, e.expected)}